...
string path_to_lua_file = "leo_ff_sl.lua";
...
const int NXD  = 2*(7);  // Number of differential states
const int NXA  = 0;  // Number of algebraic states

const int NU   = (7-3);  // Number of controls
...
void lsqfcn_height_tracking (
	...
	leo.updateState (sd, u, p, "double_support");
	...
	suppport_center += position_tip_right;
	suppport_center += position_heel_right;
	suppport_center /= 2.0;
	...
	res[res_cnt++] =   10.00 * momentum_CoM[1];
	...
	// regularize: || u ||_2^2
	// res[res_cnt++] = 0.001 * u[TAUS["arm"]]; // arm
	// res[res_cnt++] = 0.001 * u[TAUS["hip_left"]]; // hip_left
	// res[res_cnt++] = 0.001 * u[TAUS["knee_left"]]; // knee_left
	// res[res_cnt++] = 0.001 * u[TAUS["ankle_left"]]; // ankle_left
...
void ffcn (
	...
	leo.updateState (xd, u, p, "double_support");
...
const unsigned int RDFCN_N = 5, RDFCN_NE = 3;
void rdfcn(
	double *ts, double *sd, double *sa, double *u, double *p, double *pr,
	double *res, long *dpnd, InfoPtr *info
) {
	...
	// if (false) {
		...
		// std::cout << position_heel_right[0] << " <= " << CoM[0] << " <= " << position_tip_right[0] << std::endl;
		// std::cout << position_heel_right[1] << " <= " << CoM[1] << " <= " << position_heel_right[1] << std::endl;
	...
	// NOTE: For a standing motion enforce contacts with ground
	...
	//       foot_right_x,y <= com <= right_foot_x,y
	// res[res_n_cnt++] = position_tip_right[0] - CoM[0]; // >= 0
		// min(position_tip_right[0], position_heel_right[0]) - CoM[0]; // >= 0
	// res[res_n_cnt++] = CoM[0] - position_heel_right[0]; // >= 0
		// CoM[0] - max(position_tip_right[0], position_heel_right[0]); // >= 0
...
void data_in (
	...
	if (!model_loaded) {
		...
		assert (leo.nDof - 3 == leo.nActuatedDof);
